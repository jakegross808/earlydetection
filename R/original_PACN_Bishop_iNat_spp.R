...
veg_species_db_folder <-"C:/Users/sbierker/Desktop/R work/Independent Project/SIP Independent Project/PACNspList"
...
#install.packages('pacnvegetation')
...
library(dplyr)
bishop <- read.csv("C:/Users/sbierker/Desktop/R work/PACN_veg/POH_Names_Table.csv")

#format data - rename scientific name column to match RSP (raw_spp_data) and delete duplicates; add row (bishop) that indicates the source
bishop1 <- bishop %>%
...
#make scientific names and A match through different formatting options (same way as the raw_spp_data)
